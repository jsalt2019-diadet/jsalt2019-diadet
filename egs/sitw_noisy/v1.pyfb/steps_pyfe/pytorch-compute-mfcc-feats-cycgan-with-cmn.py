...
def apply_nnet(x, model, chunk_size, context, device):
    ...
    x_chunk = np.zeros((chunk_size, x.shape[1]), dtype='float32')
    num_chunks = int(np.ceil(x.shape[0]/chunk_shift))
    tbeg_in = 0
    tbeg_out = 0
    for i in range(num_chunks):
        tend_in = min(tbeg_in + chunk_size, x.shape[0])
        cur_chunk_size = tend_in-tbeg_in
        x_chunk[:cur_chunk_size] = x[tbeg_in:tend_in]
        x_chunk[cur_chunk_size:] = x[cur_chunk_size-1]
        x_i = torch.tensor(x_chunk[None,None,:]).to(device)
        y_i = model(x_i).detach().cpu().numpy()[0,0,:cur_chunk_size]
        if i == 0:
