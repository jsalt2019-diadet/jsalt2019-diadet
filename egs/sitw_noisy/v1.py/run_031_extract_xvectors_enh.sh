...
if [ "$enh_train" == "true" ];then
	...
	utils/subset_data_dir.sh \
	    --utt-list <(awk '$2>1500' data/$plda_data/utt2num_frames) \
            data/${plda_data_enh} data/${plda_data}_15s
	...
	for name in ${plda_data_enh}_15s
	do
...
if [ $stage -le 5 ]; then
    ...
    rm -rf $xvector_dir/sitw_combined_enh${enh_name}
    mkdir -p $xvector_dir/sitw_combined_enh${enh_name}
    for d in $(ls $xvector_dir | grep sitw_ | grep -v sitw_combined | grep _enh${enh_name})
    do
	cat $xvector_dir/$d/xvector.scp
    done > $xvector_dir/sitw_combined_enh${enh_name}/xvector.scp
    #check that there is not repited lines in combined xvector
    NT=$(wc -l $xvector_dir/sitw_combined_enh${enh_name}/xvector.scp | awk '{ print $1}')
    NU=$(awk '{ print $1}' $xvector_dir/sitw_combined_enh${enh_name}/xvector.scp | sort -u | wc -l | awk '{ print $1}')
    if [ $NT -gt $NU ];then
	echo "There are repited lines in $xvector_dir/sitw_combined_enh${enh_name}/xvector.scp"
