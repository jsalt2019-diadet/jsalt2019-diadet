...
be_dir=exp/be/$nnet_name/$be_enh_name
score_dir=exp/scores/$nnet_name/${be_enh_name}_testenh${enh_name}
...
if [ $stage -le 1 ]; then
    ...
    steps_be/train_be_v1.sh --cmd "$train_cmd" \
			    ...
			    $xvector_dir/${plda_data_enh}_15s/xvector.scp \
			    data/${plda_data_enh}_15s \
			    $be_dir 
fi
...
if [ $stage -le 2 ];then
	...
	(
	    ...
	    steps_be/eval_be_v1.sh --cmd "$train_cmd" --plda_type $plda_type \
				   ...
				   $xvector_dir/sitw_combined_enh${enh_name}/xvector.scp \
				   $be_dir/lda_lnorm.h5 \
				   $be_dir/plda.h5 \
