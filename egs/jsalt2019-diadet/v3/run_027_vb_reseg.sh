...
if [ $stage -le 1 ]; then
	...
	output_rttm_dir=$VB_dir/rttm/$name/rttm
	...
	VB/diarization/VB_resegmentation.sh --nj $nj --cmd "$train_cmd --mem 10G" \
					    ...
					    data/$name $init_rttm_file $VB_dir/rttm/$name \
					    $VB_dir/models/$trained_dir/diag_ubm_$num_components/final.dubm \
					    $VB_dir/models/$trained_dir/extractor_diag_c${num_components}_i${ivector_dim}/final.ie || exit 1; 
...
if [ $stage -le 2 ]; then
	...
	$train_cmd $VB_dir/$name/pyannote.log \
		   local/pyannote_score_diar.sh $name $dev_eval $VB_dir/rttm/$name/rttm &
  ...
  for name in $dsets_test
    do 
      ...
      for num in ${cols[@]}; do 
        ...
        awk -v num=$num '/TOTAL/ { printf "%.2f,", $num}' $VB_dir/rttm/$name/rttm/result.pyannote-der
      done
      echo
