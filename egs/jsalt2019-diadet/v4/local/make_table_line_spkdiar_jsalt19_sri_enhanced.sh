...
do
    res_file=$score_dir/jsalt19_spkdiar_sri_enhanced_dev${vad_suff}/plda_scores_tbest/result${conds_dev[$i]}.pyannote-der
    awk '/TOTAL/ { printf "%.2f,%.2f,%.2f,%.2f,", $2,$11,$9,$13}' $res_file
done
for((i=0;i<$num_conds_eval;i++))
do
    res_file=$score_dir/jsalt19_spkdiar_sri_enhanced_eval${vad_suff}/plda_scores_tbest/result${conds_eval[$i]}.pyannote-der
    awk '/TOTAL/ { printf "%.2f,%.2f,%.2f,%.2f,", $2,$11,$9,$13}' $res_file
done
