...
be_babytrain_dir=exp/be_diar/$nnet_name/$be_diar_babytrain_enhanced_name
be_chime5_dir=exp/be_diar/$nnet_name/$be_diar_chime5_enhanced_name
be_ami_dir=exp/be_diar/$nnet_name/$be_diar_ami_enhanced_name


#Train LDA for babytrain, enhanced
if [ $stage -le 3 ];then
    train_data_dir=jsalt19_spkdiar_babytrain_enhanced_train_gtvad
    ...
    ivector-subtract-global-mean scp:$xvector_dir/$train_data_dir/xvector.scp ark,t:- >> $be_babytrain_dir/xvector.ark.tmp
    ...
    cat $xvector_dir/${plda_diar_data}_128k/utt2spk $xvector_dir/$train_data_dir/utt2spk | sort -u > $be_babytrain_dir/utt2spk
...
# Train PLDA model for babytrain, enhanced
if [ $stage -le 4 ]; then
    train_data_dir=jsalt19_spkdiar_babytrain_enhanced_train_gtvad
    ...
    cp $xvector_dir/$train_data_dir/mean.vec $be_babytrain_dir

fi



#Train LDA for chime5, enhanced
if [ $stage -le 5 ];then
    train_data_dir=jsalt19_spkdiar_chime5_enhanced_train_gtvad
    ...
    ivector-subtract-global-mean scp:$xvector_dir/$train_data_dir/xvector.scp ark,t:- >> $be_chime5_dir/xvector.ark.tmp
    #remove duplicate key, that appears for some reason
    ...
    cat $xvector_dir/${plda_diar_data}_128k/utt2spk $xvector_dir/$train_data_dir/utt2spk | sort -u > $be_chime5_dir/utt2spk
...
# Train PLDA model for chime5, enhanced
if [ $stage -le 6 ]; then
    train_data_dir=jsalt19_spkdiar_chime5_enhanced_train_gtvad
    ...
    cp $xvector_dir/$train_data_dir/mean.vec $be_chime5_dir

fi



#Train LDA for ami, enhanced
if [ $stage -le 7 ];then
    train_data_dir=jsalt19_spkdiar_ami_enhanced_train_gtvad

    mkdir -p $be_ami_dir
    # Train a LDA model on Voxceleb+ami_train,
    echo "Train LDA for ami"
    # ind zca whitening
    copy-vector scp:$xvector_dir/$train_data_dir/xvector.scp ark:- | local/zca_whitening.py ark:- $be_ami_dir/zca_whitening_ind
    # ood zca whitening
    ...
    ivector-subtract-global-mean scp:$xvector_dir/$train_data_dir/xvector.scp ark,t:- >> $be_ami_dir/xvector.ark.tmp
    #remove duplicate key, that appears for some reason
    ...
    cat $xvector_dir/${plda_diar_data}_128k/utt2spk $xvector_dir/$train_data_dir/utt2spk | sort -u > $be_ami_dir/utt2spk
...
# Train PLDA model for ami, enhanced
if [ $stage -le 8 ]; then
    train_data_dir=jsalt19_spkdiar_ami_enhanced_train_gtvad
    ...
    cp $xvector_dir/$train_data_dir/mean.vec $be_ami_dir
