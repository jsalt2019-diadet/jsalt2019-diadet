...
def get_args():
    ...
    parser.add_argument("--normalize-output", type=str,
                        help='If true, then after reverberating and possibly adding '
                            'noise, scale so that the signal energy is the same as '
...
def augment_wav(utt, wav, dur, fg_snr_opts, bg_snr_opts, fg_noise_utts, \
    bg_noise_utts, noise_wavs, noise2dur, interval, num_opts, normalize_output):
    ...
    if len(bg_noise_utts) > 0:
        ...
        for i in range(0, num):
            ...
            noise = "wav-reverberate --normalize-output=" + normalize_output \
            + " --duration=" + dur_str + " \"" + noise_wavs[noise_utt] + "\" - |"
    ...
    if wav.strip()[-1] != "|":
        new_wav = "wav-reverberate --shift-output=true --normalize-output=" + normalize_output \
            + " " + noises_str + " " + start_times_str + " " + snrs_str + " " + wav + " - |"
    # Else if the wav is in a pipe
    else:
        new_wav = wav + " wav-reverberate --shift-output=true --normalize-output=" \
            + normalize_output + " " + noises_str + " " + start_times_str + " " + snrs_str + " - - |"
    return new_wav
...
def main():
    ...
    for line in wav_scp_file:
        ...
        new_wav = augment_wav(utt, wav, dur, fg_snrs, bg_snrs, fg_noise_utts,
            ...
            num_bg_noises, args.normalize_output)
