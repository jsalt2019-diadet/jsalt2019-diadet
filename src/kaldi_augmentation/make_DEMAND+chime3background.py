...
def read_file(filepath, asdict=True):
    ...
    with open(filepath, "rt") as fid_read:
        dictionary = OrderedDict([line.split() for line in fid_read])
    return dictionary
...
if __name__ == "__main__":
    ...
    mkdir_p(os.path.join(dir_target, "DEMAND"))
    mkdir_p(os.path.join(dir_target, "chime3background"))
    mkdir_p(os.path.join(dir_target, "chime3background_train"))
    mkdir_p(os.path.join(dir_target, "chime3background_test"))

    # DEMAND
    utts = get_matchingfiles_recursive(dir_DEMAND)
    wavscp = os.path.join(dir_target, "DEMAND", "wav.scp")
    utt2spk = os.path.join(dir_target, "DEMAND", "utt2spk")
    spk2utt = os.path.join(dir_target, "DEMAND", "spk2utt")
    ...
    wavscp = os.path.join(dir_target, "chime3background_test", "wav.scp")
    utt2spk = os.path.join(dir_target, "chime3background_test", "utt2spk")
    spk2utt = os.path.join(dir_target, "chime3background_test", "spk2utt")
    with open(wavscp, "wt") as fw, open(utt2spk, "wt") as fw1, open(spk2utt, "wt") as fw2:
        for k in keys_test:
            line = k + " " + dictionary[k] + "\n"
            fw.write(line)
