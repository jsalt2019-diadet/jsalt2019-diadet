...
def get_args():
    ...
    parser.add_argument('roomid2rt60_map', type=str)
...
def check_args(args):
    ...
    if not os.path.isfile(args.utt2uniq):
        raise ValueError('File provided utt2uniq {w} does not exist'.format(
                        w=args.utt2uniq))
    if not os.path.isfile(args.roomid2rt60_map):
        raise ValueError('File provided rt60 info file {r} does not exist'.format(
                        r=args.roomid2rt60_map))

    return args


def map_room_ids_to_rt60s(ifile):
    ...
    for line in content:
        ...
        roomid, rt60 = line_parsed[0], line_parsed[1]
        roomid_to_rt60[roomid] = rt60

    return roomid_to_rt60
...
def main():
    ...
    roomid_to_rt60 = map_room_ids_to_rt60s(args.roomid2rt60_map)
